...
int main(void) {
	...
	char text[MAX_LINES * MAX_LINE_LENGTH] = { 0 }; // Array to store copied text
	
    while (!WindowShouldClose()) {
        update_text_box(&text_box);
        
	 if (IsKeyPressed(KEY_ENTER)) {
	  // Copy content to 'text' array when Enter is pressed
	    strcpy(text, get_text_box_content(&text_box));
	    printf("TextBox Content:\n%s\n", text); // Print the copied content
	  }
        BeginDrawing();
